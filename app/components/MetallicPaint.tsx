...
export default function MetallicPaint({
  ...
  useEffect(() => {
    ...
    context.shaderSource(fs, liquidFragSource);
    ...
    const u_patternScale = context.getUniformLocation(program, "u_patternScale")!;
    const u_refraction = context.getUniformLocation(program, "u_refraction")!;
    const u_edge = context.getUniformLocation(program, "u_edge")!;
    const u_patternBlur = context.getUniformLocation(program, "u_patternBlur")!;
    const u_liquid = context.getUniformLocation(program, "u_liquid")!;
    const u_image_texture = context.getUniformLocation(program, "u_image_texture")!;

    setUniforms({ u_time, u_ratio, u_img_ratio, u_patternScale, u_refraction, u_edge, u_patternBlur, u_liquid, u_image_texture });
    setGl(context);
  }, [imageData]);
